...
Run the tests with `pytest` — **404 pass, 2 skip** (the two need the
...
406 tests — 404 pass and 2 skip where the OpenTelemetry exporter is
...
notifications, prompt-injection guardrails, billing reconciliation, a guard
against committed credentials and the API —
...
tests/                    406 tests, no credentials needed
```
